...
function doFlightCheck(): boolean {
  ...
  try {
    ...
    if (os.platform() === "win32") {
      out = cp
        .execFileSync(
          "powershell " + getCommandString(commands.flightCheck, false, false),
          { shell: true }
        )
        .toString("utf-8");
    } else {
...
function getCommandString(
  ...
  withTextSelection: boolean = true
) {
  assert(cmd.uri);
  let ret = "";
